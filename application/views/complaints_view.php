...
<body>
	...
	<div class="main" id="main">
		...
		<?php if ($this->session->flashdata('success')): ?>
			<div class="notification success"><i class="fas fa-check-circle"></i>
				<?= $this->session->flashdata('success') ?></div>
		<?php endif; ?>
		<?php if ($this->session->flashdata('error')): ?>
			<div class="notification error"><i class="fas fa-exclamation-circle"></i>
		...
		<!-- Filter bar (GET method, no AJAX) -->
		<form method="get" action="<?= site_url('complaints') ?>" id="filterForm">
			<div class="filter-section">
				...
				<div class="filter-group">
					...
					<select name="status" class="filter-select">
						...
						<?php foreach (['pending' => 'Pending', 'in-progress' => 'In Progress', 'resolved' => 'Resolved', 'closed' => 'Closed'] as $val => $label): ?>
							<option value="<?= $val ?>" <?= ($filters['status'] === $val) ? 'selected' : '' ?>><?= $label ?>
							</option>
						<?php endforeach; ?>
					</select>
				</div>
				...
				<div style="display:flex;gap:8px;align-items:flex-end;">
					...
					<a href="<?= site_url('complaints') ?>" class="btn btn-outline"><i class="fas fa-times"></i>
						Clear</a>
				</div>
			</div>
		</form>
		...
		<div class="table-section">
			<div class="table-header">
				<h3>
					...
					<small style="font-weight:400;color:var(--text-light);font-size:.82rem;">(<?= $total_count ?>
						total)</small>
				</h3>
				<div class="page-actions">
					<button class="btn btn-primary" onclick="openAddModal()"><i class="fas fa-plus-circle"></i> Register
						Complaint</button>
				</div>
			</div>
						...
						<?php if (!empty($complaints)): ?>
							<?php foreach ($complaints as $c): ?>
								<tr data-row-id="<?= (int) $c['id'] ?>">
									...
									<td><span
											class="status-badge <?= html_escape($c['status']) ?>"><?= ucwords(str_replace('-', ' ', html_escape($c['status']))) ?></span>
									</td>
									<td>
										<div class="action-buttons">
											<button class="btn-icon" title="View"
												onclick='viewComplaint(<?= htmlspecialchars(json_encode($c), ENT_QUOTES) ?>)'><i
													class="fas fa-eye"></i></button>
											<button class="btn-icon" title="Edit"
												onclick='editComplaint(<?= htmlspecialchars(json_encode($c), ENT_QUOTES) ?>)'><i
													class="fas fa-edit"></i></button>
											<button class="btn-icon delete" title="Delete"
												onclick="deleteComplaint(<?= (int) $c['id'] ?>)"><i
		...
		<div class="modal-content" style="max-width:640px;">
			...
			<form id="complaintForm" method="POST" action="<?= base_url('complaints/add') ?>"
				...
				<div class="modal-body" style="padding:16px 20px;">
								...
								<select name="member_id" id="memberSelect" class="form-control" required
									...
									<?php foreach ($members as $m): ?>
										<option value="<?= (int) $m['id'] ?>"
											...
											<?= html_escape($m['name'] ?? '') ?> (<?= html_escape($m['flat_no'] ?? '') ?>)
					...
					<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:14px;">
						...
						<div class="form-group">
							...
							<select name="status" id="formStatus" class="form-control"
								onchange="toggleResolutionFields(this.value)">
					...
					<!-- Extra fields for resolved -->
					<div class="form-group" id="resolutionExtraGroup" style="display:none;">
						<label><i class="fas fa-rupee-sign"></i> Expense Amount *</label>
						<input type="number" name="expense_amount" id="expenseAmount" class="form-control" min="0"
							step="0.01" placeholder="Enter expense amount">
					</div>

					<div class="form-group" id="resolutionImageGroup" style="display:none;">
						<label><i class="fas fa-image"></i> Upload Image *</label>
						<input type="file" name="resolution_image" id="resolutionImage" class="form-control"
							accept="image/*">
					</div>
				</div>

				<div class="modal-footer" style="display:flex;justify-content:flex-end;gap:10px;padding:14px 20px;">
					<button type="button" class="btn btn-outline" onclick="closeModal('complaintFormModal')"><i
	...
	<div class="modal" id="deleteModal">
		<div class="modal-content" style="max-width:400px;">
			...
			<div class="modal-body" style="text-align:center;padding:24px;">
				...
				<span style="color:var(--danger);font-size:.88rem;display:block;margin-top:8px;">This cannot be
					undone.</span>
			</div>
			<div class="modal-footer" style="display:flex;justify-content:flex-end;gap:10px;">
	...
	<script>
		...
		document.addEventListener('keydown', e => { if (e.key === 'Escape') { document.querySelectorAll('.modal.active').forEach(m => m.classList.remove('active')); document.getElementById('overlay').classList.remove('active'); } });

		// Toggle resolution fields
		function toggleResolutionFields(status) {
			const show = (status === 'resolved' || status === 'closed');
			document.getElementById('resolutionGroup').style.display = show ? 'block' : 'none';
			document.getElementById('resolutionExtraGroup').style.display = show ? 'block' : 'none';
			document.getElementById('resolutionImageGroup').style.display = show ? 'block' : 'none';
			document.getElementById('expenseAmount').required = show;
			document.getElementById('resolutionImage').required = show;
		}
		...
		function openAddModal() {
			...
			document.getElementById('resolutionExtraGroup').style.display = 'none';
			document.getElementById('resolutionImageGroup').style.display = 'none';
			document.getElementById('expenseAmount').required = false;
			document.getElementById('resolutionImage').required = false;
		...
		function editComplaint(c) {
			...
			document.getElementById('resolutionExtraGroup').style.display = showRes ? 'block' : 'none';
			document.getElementById('resolutionImageGroup').style.display = showRes ? 'block' : 'none';
			document.getElementById('expenseAmount').required = showRes;
			document.getElementById('resolutionImage').required = showRes;
			document.getElementById('expenseAmount').value = c.expense_amount || '';
