...
<head>
    ...
    <style>
        tr.society-group-row>td{background:#eef1fb!important;color:#3b5bdb;font-weight:700;font-size:.83rem;padding:8px 16px!important;border-top:2px solid #c5d0f5;letter-spacing:.02em;}
        tr.chairman-row{background:#fffbeb!important;border-left:3px solid #f59e0b;}
        .society-badge{display:inline-flex;align-items:center;gap:5px;background:#eef1fb;color:#3b5bdb;border:1px solid #c5d0f5;border-radius:20px;padding:2px 10px;font-size:.78rem;font-weight:500;}
        .chairman-badge{display:inline-flex;align-items:center;gap:5px;background:#fef3c7;color:#92400e;border:1px solid #fcd34d;border-radius:20px;padding:3px 10px;font-size:.78rem;font-weight:700;}
        ...
        .member-type-badge.owner{background:#d1fae5;color:#065f46;}
        .member-type-badge.tenant{background:#dbeafe;color:#1d4ed8;}
        ...
        /* ── Flat picker ── */
        .flat-picker-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(82px,1fr));gap:7px;max-height:210px;overflow-y:auto;padding:8px;border:1.5px solid #dde3ee;border-radius:10px;background:#f9fafc;}
        .fp-tile{padding:7px 5px;border-radius:8px;border:1.5px solid #dde3ee;background:#fff;cursor:pointer;transition:all .18s;font-family:'Inter',sans-serif;text-align:center;}
        .fp-tile:hover{border-color:var(--primary);background:#e8f4fd;}
        .fp-tile.selected{border-color:var(--primary);background:#dbeafe;color:#1d4ed8;}
        .fp-tile .ft-no{font-weight:800;font-size:.82rem;color:#1e293b;line-height:1.2;}
        .fp-tile .ft-type{font-size:.63rem;color:#64748b;margin-top:1px;}
        .fp-tile.selected .ft-no,.fp-tile.selected .ft-type{color:#1d4ed8;}
        .fp-floor-hdr{grid-column:1/-1;font-size:.67rem;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:.07em;padding:4px 0 2px;border-bottom:1px solid #e8f0fe;}
        .selected-flat-bar{display:none;background:#e0f2fe;border:1.5px solid #7dd3fc;border-radius:10px;padding:9px 14px;margin-bottom:10px;align-items:center;justify-content:space-between;}
        .sfb-icon{width:34px;height:34px;background:#0369a1;color:#fff;border-radius:9px;display:flex;align-items:center;justify-content:center;font-size:.9rem;flex-shrink:0;}
        .sfb-clear{background:#fee2e2;color:#dc2626;border:none;border-radius:7px;padding:4px 10px;cursor:pointer;font-size:.72rem;font-weight:600;font-family:'Inter',sans-serif;}
        ...
        .setup-prompt{background:#fffbeb;border:1px solid #fcd34d;border-radius:10px;padding:11px 14px;font-size:.82rem;color:#92400e;}

        /* ── CSV Import modal specific ── */
        .csv-format-table { width:100%; border-collapse:collapse; font-size:.78rem; margin:12px 0; }
        .csv-format-table th { background:#f1f5f9; padding:7px 10px; text-align:left; font-weight:700; color:#475569; border:1px solid #e2e8f0; }
        .csv-format-table td { padding:7px 10px; border:1px solid #e2e8f0; color:#1e293b; vertical-align:top; }
        .csv-format-table td:first-child { font-family:monospace; font-weight:700; color:#0369a1; }
        .csv-format-table td.req { color:#dc2626; font-size:.7rem; font-weight:700; }
        .csv-format-table td.opt { color:#64748b; font-size:.7rem; }

        .import-drop-zone {
            border: 2px dashed #c7d2fe; border-radius: 14px;
            padding: 28px; text-align: center;
            background: #f8f9ff; transition: all .2s; cursor: pointer;
            position: relative;
        }
        ...
        .import-drop-zone i  { font-size: 2rem; color: #c7d2fe; display: block; margin-bottom: 8px; }
        .import-drop-zone .dz-label { font-size: .88rem; color: #64748b; font-weight: 500; }
        .import-drop-zone .dz-label strong { color: var(--primary); }
        .import-drop-zone .dz-sub { font-size: .74rem; color: #94a3b8; margin-top: 4px; }
        ...
        .file-chosen i { color: #059669; }
        .file-chosen .fc-name { font-size: .84rem; font-weight: 700; color: #065f46; }
        .file-chosen .fc-size { font-size: .72rem; color: #059669; }
        ...
        .import-step {
            ...
            border-radius: 10px; padding: 10px 12px; flex: 1; min-width: 140px;
        }
        .is-num {
        ...
        .is-text { font-size: .78rem; color: #475569; font-weight: 500; }
        .is-text strong { color: #1e293b; display: block; margin-bottom: 2px; }

        /* ── Import result detail box ── */
        .import-result-box {
            background: #f8fafc; border: 1px solid #e2e8f0;
            border-radius: 12px; padding: 14px 16px; margin-top: 12px;
        }
        .import-result-box h4 { font-size: .8rem; font-weight: 700; color: #475569; margin: 0 0 8px; text-transform: uppercase; letter-spacing: .05em; }
        .ir-item { font-size: .78rem; padding: 4px 0; border-bottom: 1px solid #f1f5f9; color: #1e293b; }
        .ir-item:last-child { border-bottom: none; }
        .ir-item.err { color: #dc2626; }
        .ir-item.warn { color: #d97706; }
    </style>
</head>
<body>
...
<div class="stats-grid">
    <div class="stat-card"><div class="stat-icon"><i class="fas fa-user-friends"></i></div>
        <div class="stat-info"><h4>Total Members</h4><h2><?= $totalMembers ?></h2>
            <div class="stat-trend trend-up"><i class="fas fa-arrow-up"></i> +<?= $newThisMonth ?> this month</div></div></div>
    <div class="stat-card"><div class="stat-icon"><i class="fas fa-home"></i></div>
        <div class="stat-info"><h4>Owners</h4><h2><?= $owners ?></h2><div class="stat-trend"><?= $ownerPercent ?>%</div></div></div>
    <div class="stat-card"><div class="stat-icon"><i class="fas fa-user-tag"></i></div>
        <div class="stat-info"><h4>Tenants</h4><h2><?= $tenants ?></h2><div class="stat-trend"><?= $tenantPercent ?>%</div></div></div>
    <div class="stat-card"><div class="stat-icon"><i class="fas fa-flag"></i></div>
        <div class="stat-info"><h4>Committee</h4><h2><?= $committee ?></h2>
            <div class="stat-trend"><?= $active ?> active · <?= $inactive ?> inactive</div></div></div>
</div>
...
<div class="committee-section">
    ...
    <div class="committee-grid">
        <?php $cMembers = array_filter($members, fn($m) => !empty($m->committee_role)); ?>
        ...
        <?php else: ?>
            <?php foreach ($cMembers as $m):
                $np = explode(' ',$m->name,2); $in = strtoupper(substr($np[0],0,1).(isset($np[1])?substr($np[1],0,1):''));
                $ic = strtolower($m->committee_role)==='chairman'; ?>
                <div class="committee-card">
                    <div class="committee-avatar" style="<?= $ic?'background:#fcd34d;color:#92400e;':'' ?>"><?= $in ?></div>
                    <div class="committee-info">
                        ...
                        <div class="committee-term"><?= html_escape($m->wing_name??'') ?> <?= html_escape($m->flat_no) ?></div>
                    </div>
                </div>
            <?php endforeach; ?>
        <?php endif; ?>
...
<form id="filterForm">
    <div class="filter-section" style="flex-wrap:wrap;gap:12px;">
        ...
        <div class="filter-group"><label><i class="fas fa-user-tag"></i> Member Type</label>
            <select name="member_type" class="filter-select" onchange="this.form.submit()">
            ...
            <select name="role" class="filter-select" onchange="this.form.submit()">
                ...
                <?php foreach ($committee_roles as $role): ?>
                    <option value="<?= strtolower($role->role_name) ?>" <?= ($filters['role']===strtolower($role->role_name))?'selected':'' ?>>
                        <?= ucwords(str_replace('_',' ',$role->role_name)) ?></option>
                <?php endforeach; ?>
            </select></div>
        ...
        <div class="search-box"><i class="fas fa-search"></i>
            <input type="text" name="search" placeholder="Search name, flat, phone..." value="<?= html_escape($filters['search']??'') ?>"></div>
...
<div class="table-section">
    <div class="table-header">
        <h3><i class="fas fa-list"></i> <?= $isSuperAdmin?'All Societies — Members Directory':'Members Directory' ?>
            <small style="font-weight:400;color:var(--text-light);font-size:.8rem;">(<?= count($members) ?> records)</small></h3>
    ...
    <!-- ★ Import result detail (shown after import if there were errors/warnings) ★ -->
    <?php $importResult = $this->session->userdata('import_result'); ?>
    <?php if (!empty($importResult)): ?>
    <div style="padding:14px 20px;border-bottom:1px solid #f0f4fb;">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">
            <i class="fas fa-info-circle" style="color:var(--primary);"></i>
            <span style="font-weight:700;font-size:.88rem;">Import Result:
                <span style="color:var(--green);"><?= $importResult['inserted'] ?> inserted</span>,
                <span style="color:var(--red);"><?= $importResult['skipped'] ?> skipped</span>
            </span>
            <button onclick="this.closest('.import-result-detail').remove(); document.querySelectorAll('.import-result-detail').forEach(e=>e.remove());"
                style="margin-left:auto;background:none;border:none;cursor:pointer;color:#64748b;font-size:1rem;">×</button>
        </div>
        <?php if (!empty($importResult['errors'])): ?>
        <div class="import-result-box">
            <h4><i class="fas fa-times-circle" style="color:#dc2626;margin-right:4px;"></i>Errors (<?= count($importResult['errors']) ?>)</h4>
        ...
        <div class="import-result-box" style="margin-top:8px;">
            <h4><i class="fas fa-exclamation-triangle" style="color:#d97706;margin-right:4px;"></i>Warnings (<?= count($importResult['warnings']) ?>)</h4>
    ...
    <div class="table-wrapper">
        <table id="membersTable">
            <thead><tr>
                <th>Member</th><th>Flat</th><th>Wing</th><th>Type</th><th>Committee Role</th>
                <?php if ($isSuperAdmin): ?><th>Society</th><?php endif; ?>
                <th>Status</th><th>Actions</th>
            </tr></thead>
            <tbody>
            <?php if (empty($members)): ?>
                <tr><td colspan="<?= $isSuperAdmin?8:7 ?>" style="text-align:center;padding:32px;color:var(--text-light);">
                    <i class="fas fa-users" style="font-size:2rem;display:block;margin-bottom:8px;opacity:.4;"></i>No members found</td></tr>
            <?php elseif ($isSuperAdmin): ?>
                <?php foreach ($societyGroups as $sName => $sMembers): ?>
                    <tr class="society-group-row"><td colspan="8"><i class="fas fa-city"></i> <?= htmlspecialchars($sName) ?>
                        <span style="font-weight:400;margin-left:8px;color:#5c7cfa;">— <?= count($sMembers) ?> members</span></td></tr>
                    <?php foreach ($sMembers as $m):
                        $isCh=strtolower($m->committee_role??'')==='chairman';
                        $sLbl=$m->status?'Active':'Inactive'; $sCls=$m->status?'active':'inactive';
                        $np=explode(' ',$m->name,2); $ini=strtoupper(substr($np[0],0,1).(isset($np[1])?substr($np[1],0,1):''));
                    ?>
                    <tr class="<?= $isCh?'chairman-row':'' ?>">
                        <td><div class="member-name-cell">
                            <div class="m-avatar" style="background:<?= $isCh?'#fcd34d':'var(--primary)' ?>;color:<?= $isCh?'#92400e':'#fff' ?>;"><?= $ini ?></div>
                            <div><span style="font-weight:<?= $isCh?'700':'500' ?>;"><?= html_escape($m->name) ?></span>
                                <?php if($isCh):?><i class="fas fa-crown crown-icon"></i><?php endif;?></div></div></td>
                        ...
                        <td><?php if($isCh):?><span class="chairman-badge"><i class="fas fa-crown"></i> Chairman</span>
                            <?php elseif(!empty($m->committee_role)):?><span class="role-badge"><?= ucwords(str_replace('_',' ',html_escape($m->committee_role))) ?></span>
                            <?php else:?><span style="color:var(--text-light);">—</span><?php endif;?></td>
                        ...
                        <td><div class="action-buttons">
                            <button class="btn-icon" onclick="openModal('vm_<?= $m->id ?>')"><i class="fas fa-eye"></i></button>
                        </div></td>
                    </tr>
                    <div class="modal" id="vm_<?= $m->id ?>">
                        <div class="modal-content"><div class="modal-header"><h3><i class="fas fa-user"></i> Member Details</h3>
                            <span class="modal-close" onclick="closeModal('vm_<?= $m->id ?>')">&times;</span></div>
                            <div class="modal-body">
                                <div style="display:flex;align-items:center;gap:18px;margin-bottom:18px;">
                                    <div class="staff-avatar" style="width:68px;height:68px;font-size:1.6rem;"><?= $ini ?></div>
                                    <div><h2 style="margin:0 0 5px;"><?= html_escape($m->name) ?></h2>
                                        <span class="member-type-badge <?= $m->member_type??'' ?>"><?= ucfirst($m->member_type??'—') ?></span></div></div>
                                <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                                    <div style="background:var(--light-bg);padding:14px;border-radius:10px;">
                                        ...
                                        <p><i class="fas fa-layer-group"></i> Wing: <strong><?= html_escape($m->wing_name??'—') ?></strong></p></div>
                                    <div style="background:var(--light-bg);padding:14px;border-radius:10px;">
                                        ...
                                        <p><i class="fas fa-envelope"></i> <?= html_escape($m->email??'—') ?></p></div></div>
                            </div>
                    ...
                    <td><div class="action-buttons">
                        ...
                        <?php if (!empty($m->committee_role)): ?>
                            <form method="post" action="<?= base_url('feature_controller/remove_role') ?>" style="display:inline;">
                                ...
                                <button type="submit" class="btn-icon delete" title="Remove Role" onclick="return confirm('Remove role?')"><i class="fas fa-times-circle"></i></button></form>
                        <?php endif; ?>
                        <button class="btn-icon" title="Edit"
                            onclick='openEditMemberModal(<?= htmlspecialchars(json_encode(["id"=>$m->id,"name"=>$m->name,"flat_no"=>$m->flat_no,"wing_id"=>$m->wing_id,"member_type"=>$m->member_type,"phone"=>$m->phone,"email"=>$m->email,"status"=>$m->status]),ENT_QUOTES,"UTF-8") ?>)'>
                            <i class="fas fa-edit"></i></button>
                        <a href="<?= base_url('feature_controller/delete_member/'.$m->id) ?>" onclick="return confirm('Delete <?= html_escape($m->name) ?>?')">
                            <button class="btn-icon delete" title="Delete"><i class="fas fa-trash"></i></button></a>
                    </div></td>
                </tr>
                <!-- View Modal -->
                <div class="modal" id="vm_<?= $m->id ?>">
                    <div class="modal-content"><div class="modal-header" style="<?= $isCh?'background:#fffbeb;':'' ?>">
                        <h3><?= $isCh?'<i class="fas fa-crown" style="color:#d97706"></i> Chairman Details':'<i class="fas fa-user"></i> Member Details' ?></h3>
                        <span class="modal-close" onclick="closeModal('vm_<?= $m->id ?>')">&times;</span></div>
                        <div class="modal-body">
                            <div style="display:flex;align-items:center;gap:18px;margin-bottom:20px;">
                                <div class="staff-avatar" style="width:70px;height:70px;font-size:1.7rem;<?= $isCh?'background:#fcd34d;color:#92400e;':'' ?>"><?= $ini ?></div>
                                <div><h2 style="margin:0 0 4px;"><?= html_escape($m->name) ?><?= $isCh?' <i class="fas fa-crown" style="color:#d97706;"></i>':'' ?></h2>
                                    <span class="member-type-badge <?= $m->member_type??'' ?>"><?= ucfirst($m->member_type??'—') ?></span></div></div>
                            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                                <div style="background:var(--light-bg);padding:16px;border-radius:10px;">
                                ...
                                <div style="background:var(--light-bg);padding:16px;border-radius:10px;">
                                    ...
                                    <p><i class="fas fa-phone"></i> <?= html_escape($m->phone??'—') ?></p>
                                    <p><i class="fas fa-envelope"></i> <?= html_escape($m->email??'—') ?></p></div></div>
                            <div style="margin-top:14px;background:var(--light-bg);padding:12px;border-radius:10px;">
                                <?php if($isCh):?><p><i class="fas fa-crown" style="color:#d97706;"></i> Role: <span class="chairman-badge"><i class="fas fa-crown"></i> Chairman</span></p>
                                <?php elseif(!empty($m->committee_role)):?><p><i class="fas fa-users-cog" style="color:var(--primary);"></i> Role: <span class="role-badge"><?= ucwords(str_replace('_',' ',html_escape($m->committee_role))) ?></span></p><?php endif;?>
                                <p><i class="fas fa-circle" style="color:var(--primary);"></i> Status: <span class="status-badge <?= $sCls ?>"><?= $sLbl ?></span></p></div></div>
                        <div class="modal-footer"><button class="btn btn-outline" onclick="closeModal('vm_<?= $m->id ?>')"><i class="fas fa-times"></i> Close</button></div>
                    </div>
                </div>
                <?php endforeach; ?>
        ...
        <div class="modal-body">
            ...
            <div class="import-steps">
                <div class="import-step">
                    ...
                    <div class="is-text"><strong>Download Sample</strong>Use the template to fill in member data correctly.</div>
                </div>
                <div class="import-step">
                    <div class="is-num">2</div>
                    <div class="is-text"><strong>Fill Data</strong>Enter all member details in the correct columns.</div>
                </div>
                <div class="import-step">
                    <div class="is-num">3</div>
                    <div class="is-text"><strong>Upload & Import</strong>Upload the CSV — members are auto-created.</div>
                </div>
            </div>
            ...
            <table class="csv-format-table">
                <thead>
                    <tr><th>Column Name</th><th>Required</th><th>Format / Example</th><th>Notes</th></tr>
                </thead>
                <tbody>
            ...
            <div style="background:#fef3c7;border:1px solid #fcd34d;border-radius:10px;padding:10px 14px;margin-bottom:16px;font-size:.79rem;color:#92400e;">
                <i class="fas fa-lightbulb" style="margin-right:4px;"></i>
                <strong>Tips:</strong> Flat must already exist and be vacant. Duplicate phone/email rows are skipped. One owner per flat is enforced. Each member automatically gets their flat marked as occupied.
            </div>

            <!-- Download sample -->
            <a href="<?= base_url('feature_controller/download_member_sample') ?>"
               class="btn btn-outline" style="margin-bottom:16px;">
                <i class="fas fa-download"></i> Download Sample CSV
            </a>

            <!-- File upload form -->
            <form id="importForm" method="post"
                  action="<?= base_url('feature_controller/import_members') ?>"
                ...
                <div class="file-chosen" id="fileChosen">
                    ...
                    <button type="button" onclick="clearFileChosen()"
                        style="margin-left:auto;background:none;border:none;cursor:pointer;color:#059669;font-size:1rem;">
    ...
    <div class="modal-content" style="max-width:660px;">
        ...
        <form id="memberForm" method="post" action="<?= base_url('feature_controller/save') ?>">
            ...
            <div style="font-size:.7rem;font-weight:700;letter-spacing:.1em;color:var(--text-light);text-transform:uppercase;display:flex;align-items:center;gap:10px;margin-bottom:12px;">
                <span style="flex:1;height:1px;background:#e8f0fe;"></span>Personal Info<span style="flex:1;height:1px;background:#e8f0fe;"></span></div>
            <div class="form-row">
                ...
                <div class="form-group"><label>Member Type *</label>
                    <select name="member_type" class="form-control" id="memberType" required>
                        <option value="owner">Owner</option><option value="tenant">Tenant</option></select></div>
            ...
            <div id="flatPickerSection">
                <div style="font-size:.7rem;font-weight:700;letter-spacing:.1em;color:var(--text-light);text-transform:uppercase;display:flex;align-items:center;gap:10px;margin:12px 0;">
                    <span style="flex:1;height:1px;background:#e8f0fe;"></span>Flat Assignment<span style="flex:1;height:1px;background:#e8f0fe;"></span></div>
                ...
                <div class="setup-prompt">
                    ...
                    <?php if (in_array(strtolower($this->session->userdata('role_name')),['chairman','super_admin'])): ?>
                ...
                <div class="form-row" style="margin-bottom:10px;">
                    ...
                    <div class="form-group">
                        ...
                        <select id="fpFloor" class="form-control" onchange="fpFilter()" disabled><option value="">All Floors</option></select>
                    </div>
                </div>
                <div class="selected-flat-bar" id="selFlatBar">
                    <div style="display:flex;align-items:center;gap:10px;">
                        <div class="sfb-icon"><i class="fas fa-door-closed"></i></div>
                        <div><div id="selFlatNo" style="font-weight:800;font-size:1rem;color:#0c4a6e;"></div>
                            <div id="selFlatMeta" style="font-size:.73rem;color:#0369a1;"></div></div>
                    </div>
                    <button type="button" class="sfb-clear" onclick="fpClear()"><i class="fas fa-times"></i> Clear</button>
                </div>
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">
                    <label style="font-size:.8rem;font-weight:600;color:var(--text-dark);margin:0;">
                        <i class="fas fa-th" style="color:var(--primary);margin-right:3px;"></i> Available Flats</label>
                    <span id="fpCount" style="background:#d1fae5;color:#065f46;border-radius:20px;padding:1px 9px;font-size:.7rem;font-weight:600;"><?= count($vacantFlats) ?> vacant</span>
                </div>
                ...
                <div id="fpEmpty" style="display:none;text-align:center;padding:22px;color:var(--text-light);font-size:.82rem;">
                    <i class="fas fa-door-open" style="display:block;font-size:1.5rem;opacity:.3;margin-bottom:5px;"></i>No vacant flats for this selection.</div>
                <?php endif; ?>
            </div>
            ...
            <div style="font-size:.7rem;font-weight:700;letter-spacing:.1em;color:var(--text-light);text-transform:uppercase;display:flex;align-items:center;gap:10px;margin:12px 0 10px;">
                <span style="flex:1;height:1px;background:#e8f0fe;"></span>Login Credentials<span style="flex:1;height:1px;background:#e8f0fe;"></span></div>
            <div class="form-group">
...
<div class="modal" id="committeeModal">
    <div class="modal-content"><div class="modal-header"><h3><i class="fas fa-users-cog"></i> Assign Committee Role</h3>
        <span class="modal-close" onclick="closeModal('committeeModal')">&times;</span></div>
        <div class="modal-body">
...
function handleFileChosen(input) {
    ...
    if (!file.name.toLowerCase().endsWith('.csv')) {
        alert('Only .csv files are allowed.'); input.value = ''; return;
    }
    if (file.size > 2 * 1024 * 1024) {
        alert('File too large. Maximum size is 2 MB.'); input.value = ''; return;
    }
    document.getElementById('fcName').textContent = file.name;
    document.getElementById('fcSize').textContent = (file.size / 1024).toFixed(1) + ' KB · ' + file.lastModifiedDate?.toLocaleDateString();
...
/* ════════════════════════════════════════════════
   FLAT PICKER
════════════════════════════════════════════════ */
let allFlats = <?= json_encode(array_map(function($f){
    return ['id'=>(int)$f->id,'flat_no'=>$f->flat_no,'floor'=>(int)$f->floor,
            'flat_type'=>$f->flat_type,'wing_id'=>(int)($f->wing_id??0),
            'wing_name'=>$f->wing_name??'','floor_label'=>$f->floor_label??''];
}, $vacantFlats ?? [])) ?>;
let filteredFlats = [...allFlats];
let pickedFlat = null;

function fpRender(flats) {
    const grid=document.getElementById('fpGrid'); const empty=document.getElementById('fpEmpty'); const cnt=document.getElementById('fpCount');
    if (!grid) return;
    grid.innerHTML='';
    if (!flats.length) { grid.style.display='none'; if(empty)empty.style.display='block'; if(cnt)cnt.textContent='0 vacant'; return; }
    grid.style.display='grid'; if(empty)empty.style.display='none'; if(cnt)cnt.textContent=flats.length+' vacant';
    const byFloor={};
    flats.forEach(f=>{ const k=f.floor_label||(f.floor===0?'Ground Floor':f.floor+' Floor'); if(!byFloor[k])byFloor[k]=[]; byFloor[k].push(f); });
    Object.entries(byFloor).forEach(([lbl,fls])=>{
        const hdr=document.createElement('div'); hdr.className='fp-floor-hdr';
        hdr.innerHTML=`<i class="fas fa-stairs" style="margin-right:3px;color:var(--primary);"></i>${lbl}`; grid.appendChild(hdr);
        fls.forEach(f=>{ const t=document.createElement('button'); t.type='button'; t.className='fp-tile'; t.dataset.id=f.id;
...
function fpClear() {
    pickedFlat=null; document.getElementById('hFlatNo').value=''; document.getElementById('hWingId').value=''; document.getElementById('hFlatId').value='';
    document.getElementById('selFlatBar').style.display='none';
    document.querySelectorAll('.fp-tile.selected').forEach(t=>t.classList.remove('selected'));
}
function fpFilter() {
    const wingId=parseInt(document.getElementById('fpWing')?.value||0)||0;
    const floorSel=document.getElementById('fpFloor'); const prevFloor=floorSel?.value||'';
    const seen=new Set(); const floors=[];
    allFlats.filter(f=>!wingId||f.wing_id===wingId).forEach(f=>{ if(!seen.has(f.floor)){seen.add(f.floor);floors.push({floor:f.floor,label:f.floor_label});} });
    floors.sort((a,b)=>a.floor-b.floor);
    if(floorSel){ floorSel.innerHTML='<option value="">All Floors</option>';
        floors.forEach(fl=>{ const o=document.createElement('option'); o.value=fl.floor; o.textContent=fl.label; if(String(fl.floor)===prevFloor)o.selected=true; floorSel.appendChild(o); });
        floorSel.disabled=floors.length===0; }
    filteredFlats=allFlats.filter(f=>{ const wm=!wingId||f.wing_id===wingId; const fm=(floorSel?.value??'')==''||String(f.floor)===String(floorSel?.value??''); return wm&&fm; });
    fpRender(filteredFlats); fpClear();
}

/* ── Add / Edit member modals ── */
function openAddMemberModal() {
    document.getElementById('memberForm').reset(); document.getElementById('memberId').value='';
    ...
    document.getElementById('memberType').value='owner'; document.getElementById('statusSel').value='1';
    ...
    if(document.getElementById('fpWing')) document.getElementById('fpWing').value='';
    if(document.getElementById('fpFloor')){ document.getElementById('fpFloor').innerHTML='<option value="">All Floors</option>'; document.getElementById('fpFloor').disabled=true; }
...
function openEditMemberModal(m) {
    ...
    document.getElementById('firstName').value=parts[0]??''; document.getElementById('lastName').value=parts.slice(1).join(' ')||'';
    document.getElementById('memberType').value=m.member_type??'owner'; document.getElementById('phone').value=m.phone??'';
    document.getElementById('email').value=m.email??''; document.getElementById('statusSel').value=String(m.status??'1');
    document.getElementById('password').value=''; document.getElementById('hFlatNo').value=m.flat_no??''; document.getElementById('hWingId').value=m.wing_id??'';
    const fps=document.getElementById('flatPickerSection'); if(fps)fps.style.display='none';
    openModal('memberFormModal');
}
function openAssignRoleModal(id) { openModal('committeeModal'); document.getElementById('committeeMemberSelect').value=id||''; document.getElementById('committeeRoleSelect').value=''; }
...
document.getElementById('memberForm')?.addEventListener('submit', function(e) {
    ...
    if (!isEdit && document.getElementById('flatPickerSection')?.style.display!=='none') {
        if (!document.getElementById('hFlatNo').value) {
            e.preventDefault(); const g=document.getElementById('fpGrid');
            if(g){g.style.borderColor='#dc2626'; setTimeout(()=>g.style.borderColor='#dde3ee',2000);} alert('Please select a flat.'); return;
        }
    }
