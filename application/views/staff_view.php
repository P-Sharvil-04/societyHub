...
<head>
	...
	<title>SocietyHub · Staff Management</title>
...
<body>
	...
	<div class="main" id="main">
		...
		<!-- Stats — all numbers scoped to active filters -->
			...
			<div class="filter-section">
				...
				<div class="filter-group">
					...
					<select name="status" class="filter-select" onchange="this.form.submit()">
						...
						<option value="on-leave" <?= ($filters['status'] === 'on-leave') ? 'selected' : '' ?>>On Leave
						</option>
					</select>
				</div>
				...
				<div style="display:flex;gap:8px;align-items:flex-end;">
					...
					<?php
					$anyFilter = !empty($filters['search']) || !empty($filters['designation'])
						|| ($filters['status'] !== '') || !empty($filters['society_id']);
					if ($anyFilter): ?>
						<a href="<?= site_url('staff') ?>" class="btn btn-outline">
		...
		<div class="table-section">
			<div class="table-header">
				...
				<a href="<?= site_url('staff') ?>" class="btn btn-outline" title="Clear filters &amp; refresh"
						...
						<?php if (!empty($staff)): ?>
							<?php foreach ($staff as $s): ?>
								<tr>
									...
									<td>
										<span class="status-badge <?= html_escape($s['status']) ?>">
											<?= ucfirst(html_escape($s['status'])) ?>
										</span>
									</td>
									<?php if (!empty($canManage)): ?>
										<td>
											<div class="action-buttons">
												<button class="btn-icon" onclick="viewStaff(<?= (int) $s['id'] ?>)" title="View">
													<i class="fas fa-eye"></i>
												</button>
												<button class="btn-icon" onclick="editStaff(<?= (int) $s['id'] ?>)" title="Edit">
							...
							<tr>
								<td colspan="<?= !empty($isSuperAdmin) ? (!empty($canManage) ? 9 : 8) : (!empty($canManage) ? 8 : 7) ?>"
									...
									<i class="fas fa-users"
										style="font-size:2rem;margin-bottom:10px;display:block;opacity:.4"></i>
									No staff found
								</td>
							</tr>
						<?php endif; ?>
				...
				<?php if (!empty($recent_staff)): ?>
					<?php foreach ($recent_staff as $r): ?>
						<div class="member-item">
							...
							<span class="member-status status-<?= html_escape($r['status']) ?>">
								<?= ucfirst(html_escape($r['status'])) ?>
							</span>
						</div>
						<br>
					<?php endforeach; ?>
		...
		<div class="modal" id="staffFormModal">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form id="staffForm" method="post" action="<?= base_url('staff/save') ?>">
	...
	<script>
		...
		/* Export CSV — all currently visible rows */
		function exportCSV() {
			var rows = [], headers = [];
			$('#staffTable thead th').each(function () { var t = $(this).text().trim(); if (t !== 'Actions') headers.push('"' + t + '"'); });
			...
			var a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'staff_export.csv'; a.click();
		...
		function saveStaff() {
			...
			fetch(form.action, {
				...
				.catch(function () { showNotification('Server error', 'error'); });
		}
		...
		function confirmDelete() {
			...
			}, 'json').fail(function () { showNotification('Delete request failed', 'error'); });
		}
