 ...
 * Flat_unit_model
 *
 * flats.status tinyint convention (matches Society_setup_model):
 *   0 = occupied
 *   1 = vacant
 *   2 = blocked
 *
 * The SELECT uses a CASE expression to convert these integers into
 * the strings 'occupied' / 'vacant' / 'blocked' so the view and CSS
 * classes work without any changes.
 */
class Flat_unit_model extends CI_Model
{
    ...
    /** Convert string label → tinyint for DB writes */
    ...
    /** SQL fragment that converts tinyint status → string label */
    ...
    /* ══════════════════════════════════════════════
       READ — All flats (with joined resident info)
    ══════════════════════════════════════════════ */
    public function get_flats(array $filters = [], bool $isSuper = false): array
    {
        $this->db->select("
            f.id,
            f.society_id,
        ...
        /*
         * Join member who lives in this flat (active users whose
         * flat_no + wing_id match).  No parking join here — parking
         * table does not have flat_no / wing_id columns.
         */
        $this->db->join(
            'users m',
            'm.flat_no    = f.flat_no
        ...
        /* Status filter — accept both string label and tinyint */
    ...
    public function get_unassigned_members(int $societyId = 0): array
    {
        ...
        /* Exclude members whose flat is already occupied (status = 0) */
        $this->db->where(
            "NOT EXISTS (
    ...
    public function get_stats(array $filters = []): array
    {
        ...
        /* By status (returns integer keys 0,1,2) */
        ...
        $this->db->where('f.status', 0); // occupied
        if ($societyId > 0) $this->db->where('f.society_id', $societyId);
    ...
    {
        /* Normalise status if passed as string */
    ...
    /* ══════════════════════════════════════════════
       WRITE — Assign resident → marks flat occupied
    ══════════════════════════════════════════════ */
